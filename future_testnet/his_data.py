...
def insert_data(conn, coin, data):
    try:
        ...
        cursor.executemany(f'''
            INSERT INTO {coin}_funding_rate (timestamp, close_contract, close_spot, funding_rate)
            VALUES (?, ?, ?, ?)
        ''', data)
        conn.commit()
...
# for index, row in results[checkout].head(30).iterrows():  # Displaying only the first 10 rows as an example
#     date_str = index.strftime('%Y-%m-%d %H:%M:%S')
#     funding_rate = row['fundingRate']
#     print(f"{date_str} | Funding Rate: {funding_rate * 1000:.4f}")